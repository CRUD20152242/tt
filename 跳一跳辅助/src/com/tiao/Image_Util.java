	...
	public int getRgb(String image, String image2) {
		...
		for (int i = height / 3; i < height * 2 / 3; i += 50) {
			...
			for (int j = 1; j < width; j++) {
				...
				rgb[2] = (pixel & 0xff);
				if (rgb[0] != last_rgb[0] || rgb[1] != last_rgb[1] || rgb[2] != last_rgb[2]) {
					scan_start_y = i - 50;
					break;
				}
		...
		for (int i = scan_start_y; i < height * 2 / 3; i++) {
			...
			for (int j = scan_x_border; j < width - scan_x_border; j++) {
				...
				rgb[1] = (pixel & 0xff00) >> 8;
				rgb[2] = (pixel & 0xff);
				if ((50 < rgb[0] && rgb[0] < 60) && (53 < rgb[1] && rgb[1] < 60) && (95 < rgb[2] && rgb[2] < 110)) {
					piece_x_sum += j;
					piece_x_c += 1;
					if (i > piece_y_max) {
						piece_y_max = i;
		...
		System.out.println("piece_x==" + piece_x + "    piece_y===" + piece_y);
		// --------------------------------------------------------------------
...
//					continue;
				...
				if (Math.abs(rgb[0] - last_rgb[0]) + Math.abs(rgb[1] - last_rgb[1])
		...
		//
		distance = (int) Math.sqrt((board_x - piece_x) *(board_x - piece_x) + (board_y - piece_y) *(board_y - piece_y));
	...
	public static void main(String[] args) {
		new Image_Util().getRgb("F:\\pic\\tiaoyitiao.png",
				"F:\\pic\\tiaoyitiao2.png");
	}
